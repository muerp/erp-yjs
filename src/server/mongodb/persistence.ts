...
export class MongodbPersistence {
  ...
  * Use this to temporarily create a Yjs document to sync changes or extract data.
  */
  getHistoryYDoc(docName: string): Promise<Y.Doc> {
    return this._transact(docName, async (db: MongoAdapter) => {
      const updates = await U.getMongoUpdates(db, docName)
      // console.log('---updates', updates.length)
      const ydoc = new Y.Doc()
  ...
  async saveHistory(docName: string, ydoc: Y.Doc) {
    const historyName = docName + '-history'
    return this._transact(historyName, db => U.flushHistoryDocument(db, historyName, Y.encodeStateAsUpdate(ydoc), Y.encodeStateVector(ydoc)))
  }

  historyUpdate(docName: string, update: Uint8Array) {
    const historyName = docName + '-history'
    return this._transact(historyName, db => U.storeUpdateHistory(db, historyName, update))
    // return this._transact(historyName, db => U.storeSnapshot(db, historyName, update))
  }

  queryHistory(docName: string, opt: any) {
    const historyName = docName
    return this._transact(historyName, async (db: MongoAdapter) => {
      const updates = await U.getMongoHistoryUpdates(db, historyName, opt)
      return updates
    })
  }
  getHistory(docName: string, query: any) {
    return this._transact(docName, async (db: MongoAdapter) => {
      return await U.queryHistoryUpdates(db, docName, query)
    })
  }
  async recoverDocument(docName: string, clock: number, doc?: Y.Doc) {
    const historyName = docName + '-history';
    return this._transact(historyName, async (db: MongoAdapter) => {
      const updates = await U.queryHistoryUpdates(db, historyName, { $or: [{ version: 'v1_sv' }, { clock: { $lte: clock } }] })
      // const updates = await U.queryHistoryUpdates(db, historyName, { clock: { $lte: clock } })
      await db.dropCollection(docName)
      updates.forEach(update => {
        update.docName = docName
      })
      await db.insertMany({docName}, updates)

      return updates.map(update => update.value);

      // if (doc) {
      //   console.log('-recoverDocument--')
      //   const ydoc = new Y.Doc()
      //   ydoc.transact(() => {
