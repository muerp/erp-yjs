...
const _convertMongoHistoryUpdates = (docs: WithId<Document>[]) => {
  if (!Array.isArray(docs) || !docs.length) return []

  return docs.map(update => ({ buffer: Array.from(update.value.buffer), time: update.time, clock: update.clock }))
}

export const getMongoHistoryUpdates = async (adapter: MongoAdapter, docName: string, opts: any = {}) => {
  const docs = await getMongoBulkData(adapter, createDocumentUpdateKey(docName), opts)
  return _convertMongoHistoryUpdates(docs)
}

export const queryHistoryUpdates = async (adapter: MongoAdapter, docName: string, query: any = {}) => {
  const docs = await getMongoBulkData(adapter, {
    docName,
    ...query
  }, {})
  if (!Array.isArray(docs) || !docs.length) return []
  return docs.map(update => ({value: update.value.buffer, clock: update.clock, action: update.action, version: update.version, docName: ''}));
}
...
export const storeSnapshot = async (adapter: MongoAdapter,
  docName: string,
  update: Uint8Array
): Promise<void> => {
  await adapter.put({
...
export const storeUpdateHistory = async (
  adapter: MongoAdapter,
  docName: string,
  update: Uint8Array,
): Promise<number> => {
...
export const flushHistoryDocument = async (
  adapter: MongoAdapter,
  docName: string,
  stateAsUpdate: Uint8Array,
  stateVector: Uint8Array,
