...
import { messageAwareness, messageRecover, messageSubDocSync, messageSync } from '../message'
import { toArray } from 'lib0/set'
...
const updateHandler = (update: Uint8Array, origin: string, doc: WSSharedDocInterface) => {
  ...
  // console.log('----check---9999', message.length)
  console.log('---updatev2-', update.length)
}
...
export const getYDoc = (
  ...
  map.setIfUndefined(docs, docname, () => {
    ...
    if (persistence !== null) {
      persistence.bindState(docname, doc, initialValue, options)
    }
...
export const closeConn = (doc: WSSharedDocInterface, conn: WebSocket.WebSocket) => {
...
export const setupWSConnection = (
  ...
  const s = url.parse(req.url!, true)
  const {
  ...
  const doc = getYDoc(docName, gc, initialValue, callback, s.query)
...
export const sendRecover = (doc: WSSharedDocInterface, conn: WebSocket.WebSocket) => {
  const encoder = encoding.createEncoder()
  encoding.writeVarUint(encoder, messageRecover)
  encoding.writeAny(encoder, {})
  send(doc, conn, encoding.toUint8Array(encoder))
}
